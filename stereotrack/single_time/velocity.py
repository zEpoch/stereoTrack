...
from .utils import kmeans_centers, nearest_neighbors, build_spatial_graph, validate_spatial_coords
try:
    ...
    GNN_AVAILABLE = False
    warnings.warn("GNN models not available. Install torch-geometric for GNN support.")
...
def get_ot_matrix(
    ...
    Supports GNN embeddings and Delaunay triangulation for 3D data.
    ...
    random_state
        ...
        (Default: 0)
    n_pcs
        The number of used pcs (only used if use_gnn=False).
        (Default: 50)
    use_gnn
        Whether to use GNN embeddings instead of PCA. Requires torch-geometric.
        (Default: False)
    use_delaunay
        Whether to use Delaunay triangulation for spatial graph (3D) or kNN (2D).
        (Default: False)
    gnn_key
        Key in ``adata.obsm`` to read/store GNN embeddings when ``use_gnn=True``.
        (Default: "X_gnn")
    ...
    if "X_gnn" not in adata.obsm:
        print("X_pca is not in adata.obsm, automatically do PCA first.")
        adata = run_3d_gnn_imputation(adata)
    newdata = adata.obsm["X_gnn"]
    newdata2 = newdata.copy()

    def getM(alpha1, alpha2):
        if data_type == "spatial":
            newcoor = adata.obsm["X_spatial"]
            # Check dimension: support both 2D (x,y) and 3D (x,y,z)
            if newcoor.shape[1] not in [2, 3]:
                raise ValueError(f"Spatial coordinates must be 2D (x,y) or 3D (x,y,z), got {newcoor.shape[1]}D")
            newcoor2 = newcoor.copy()

            # Build spatial graph if use_delaunay
            if use_delaunay:
                spatial_adj = build_spatial_graph(newcoor, method='delaunay' if newcoor.shape[1] == 3 else 'delaunay')
                # Use graph-based distance instead of Euclidean
                from scipy.sparse.csgraph import shortest_path
                dist_matrix = shortest_path(spatial_adj, directed=False, unweighted=False)
                dist_matrix[dist_matrix == np.inf] = dist_matrix[dist_matrix != np.inf].max() * 2
                ed_coor = dist_matrix ** 2
            else:
                # calculate physical distance (works for both 2D and 3D)
