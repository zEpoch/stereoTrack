...
def distance_point_to_segment(point, segment_start, segment_end):
    ...
    if projection < 0:
        ...
        # For 2D: np.cross returns scalar, for 3D: returns vector
        if len(point) == 2:
            # 2D case
            distance = np.abs(np.cross(segment_vector, point_vector)) / np.linalg.norm(segment_vector)
        else:
            # 3D case
            cross_product = np.cross(segment_vector, point_vector)
            distance = np.linalg.norm(cross_product) / np.linalg.norm(segment_vector)
...
def map_cell_to_LAP(adata,basis='spatial',cell_neighbors=150):
    ...
    if LAP_points.shape[1] != coords.shape[1]:
        raise ValueError(f"LAP points dimension ({LAP_points.shape[1]}D) doesn't match coordinate dimension ({coords.shape[1]}D)")
...
def plot_least_action_path(adata,basis='spatial',ax=None,
    ...
    Supports both 2D and 3D spatial coordinates.
    Can use k3d or plotly for advanced 3D visualization.
    ...
    basis
        ...
        (Default: 'spatial')
    ax
        Figure axes. For 3D, should be a 3D axes (ax = fig.add_subplot(111, projection='3d')).
        (Default: None)
    linewidth
    ...
    point_size
        ...
        (Default: 6)
    linestyle
        Linestyle of the LAP.
        (Default: 'solid')
    use_k3d
        Use k3d for 3D visualization (interactive, high-quality).
        (Default: False)
    use_plotly
        Use plotly for 3D visualization (interactive, web-based).
        (Default: False)

    Returns
    -----------
    ax or k3d.Plot or plotly.graph_objects.Figure
    ...
    # Check if 3D coordinates
    is_3d = lap_point_pos.shape[1] == 3
    
    # Use advanced 3D visualization if requested
    if is_3d and use_k3d:
        try:
            from .visualization_3d import plot_lap_3d_k3d
            coords = adata.obsm["X_"+basis]
            return plot_lap_3d_k3d(
                coords=coords,
                lap_path=lap_point_pos,
                lap_values=lap_value,
                point_size=point_size / 10.0,  # Adjust scale for k3d
                line_width=linewidth / 2.0,
                color_map='hsv'
            )
        except ImportError:
            warnings.warn("k3d not available, falling back to matplotlib.")
            use_k3d = False
    
    if is_3d and use_plotly:
        try:
            from .visualization_3d import plot_lap_3d_plotly
            coords = adata.obsm["X_"+basis]
            return plot_lap_3d_plotly(
                coords=coords,
                lap_path=lap_point_pos,
                lap_values=lap_value,
    ...
    if is_3d:
        ...
        ax.plot(lap_point_pos[:, 0], lap_point_pos[:, 1], lap_point_pos[:, 2], 
                c="k", linewidth=linewidth, linestyle=linestyle, zorder=3)
        ax.scatter(lap_point_pos[:, 0], lap_point_pos[:, 1], lap_point_pos[:, 2], 
                   c=cols, s=point_size, zorder=2)
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
    else:
