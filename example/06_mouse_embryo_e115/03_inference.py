...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run inference for E11.5 mouse embryo MAE checkpoints.")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("/home/share/huadjyin/home/zhoutao3/tracks/stereoTrack/config/06_config_mouse_embryo_e115.yaml"),
    )
    parser.add_argument("--checkpoint", type=Path, required=True)
    parser.add_argument("--batch-size", type=int, default=None)
    parser.add_argument("--device", type=str, default="cuda:0")
    parser.add_argument("--output-dir", type=Path, default=None)
    parser.add_argument("--genes", nargs="*", default=None, help="Optional gene subset to write into X and layers.")
    parser.add_argument("--gene-file", type=Path, default=None, help="Optional text file with one gene per line or comma-separated genes.")
    parser.add_argument("--all-genes", action="store_true", help="Write all genes into X and layers. This can create very large output files.")
    parser.add_argument("--embeddings-only", action="store_true", help="Only write cell/niche embeddings, ignoring configured output genes.")
    parser.add_argument("--write-niche-layer", action="store_true", help="Deprecated; niche layer is written whenever genes are requested.")
...
def load_meta(cfg: dict) -> tuple[dict, str]:
    ...
    cache_dir = os.path.join(cfg["paths"]["input_dir"], cache_name)
...
def requested_genes(args: argparse.Namespace, cfg: dict) -> list[str] | None:
    if args.embeddings_only or args.all_genes:
        return None

    genes: list[str] = []
    configured = cfg.get("inference", {}).get("output_genes", [])
    if configured:
        genes.extend(str(gene) for gene in configured)
...
def common_genes_from_meta(meta: dict) -> list[str]:
    common_genes = list(meta.get("common_genes") or meta.get("common_human_genes") or [])
    if common_genes:
        return common_genes
    return [f"gene_{idx}" for idx in range(int(meta["input_dim"]))]


def select_gene_indices(meta: dict, genes: list[str] | None, all_genes: bool) -> tuple[np.ndarray | slice, list[str], bool]:
    common_genes = common_genes_from_meta(meta)
    if all_genes:
        return slice(None), common_genes, True
    if genes is None:
    ...
    if missing:
        print(f"Warning: {len(missing)} requested genes are not in common_genes: {missing[:20]}")
    ...
    return np.asarray([gene_to_idx[gene] for gene in present], dtype=np.int64), present, True
...
def resolve_cached_adata(cache_dir: str, slice_idx: int, slice_info: dict) -> str | None:
    batch_name = Path(str(slice_info.get("batch", ""))).name
    candidates = []
    if slice_info.get("adata_file"):
        candidates.append(Path(cache_dir) / str(slice_info["adata_file"]))
    if batch_name:
        candidates.append(Path(cache_dir) / "adatas" / batch_name)
    candidates.extend(
        [
            Path(cache_dir) / "adatas" / f"slab_{slice_idx:03d}.h5ad",
            Path(cache_dir) / "adatas" / f"slice_{slice_idx}.h5ad",
        ]
    )
...
def build_output_adata(
    ...
    x_niche: np.ndarray | None,
    x_cell: np.ndarray | None,
    z_cell: np.ndarray,
    z_niche: np.ndarray,
    slice_info: dict,
) -> ad.AnnData:
    adata_cache = resolve_cached_adata(cache_dir, slice_idx, slice_info)
    if adata_cache is not None:
        adata_cached = sc.read_h5ad(adata_cache)
        if adata_cached.n_obs != z_cell.shape[0]:
            raise ValueError(
                f"Cached metadata rows ({adata_cached.n_obs}) do not match embeddings "
                f"({z_cell.shape[0]}) for {slice_info.get('batch', slice_idx)}"
            )
        obs = adata_cached.obs.copy()
        if selected_genes:
            var = adata_cached.var.loc[selected_genes].copy()
            x = x_input.astype(np.float32, copy=False)
        else:
            var = adata_cached.var.iloc[0:0].copy()
            x = sp.csr_matrix((adata_cached.n_obs, 0), dtype=np.float32)
        ...
        print(f"  metadata source: {adata_cache}; obsm={list(adata_cached.obsm.keys())}")
    else:
        if selected_genes:
            adata_out = ad.AnnData(x_input.astype(np.float32, copy=False))
            adata_out.var_names = selected_genes
        else:
            adata_out = ad.AnnData(sp.csr_matrix((z_cell.shape[0], 0), dtype=np.float32))

    adata_out.obsm["cell_embedding"] = z_cell.astype(np.float32, copy=False)
    adata_out.obsm["niche_embedding"] = z_niche.astype(np.float32, copy=False)
    if selected_genes and x_niche is not None:
        adata_out.layers["niche_recon"] = x_niche.astype(np.float32, copy=False)
    if selected_genes and x_cell is not None:
        adata_out.layers["cell_recon"] = x_cell.astype(np.float32, copy=False)
...
def inference() -> None:
    ...
    model_cfg = cfg["model"]
    batch_size = args.batch_size or int(train_cfg.get("infer_batch_size", 1024))
    selected_idx, selected_genes, write_gene_outputs = select_gene_indices(meta, requested_genes(args, cfg), args.all_genes)
    ...
    if not write_gene_outputs:
        print("gene output disabled: writing embeddings only")
        if args.write_cell_layer:
            print("Warning: --write-cell-layer is ignored unless --genes, --gene-file or --all-genes is used.")
    ...
    for slice_idx, slice_info in enumerate(meta["slice_info"]):
        ...
        z_cell_np = np.empty((n_cells, latent_dim), dtype=np.float32)
        z_niche_np = np.empty((n_cells, latent_dim), dtype=np.float32)
        x_input_np = np.empty((n_cells, len(selected_genes)), dtype=np.float32) if write_gene_outputs else None
        x_niche_np = np.empty((n_cells, len(selected_genes)), dtype=np.float32) if write_gene_outputs else None
        x_cell_np = np.empty((n_cells, len(selected_genes)), dtype=np.float32) if write_gene_outputs and args.write_cell_layer else None
        ...
        for start in range(0, n_cells, batch_size):
            ...
            feat_np = feat_sparse[sub_nodes].toarray().astype(np.float32, copy=False)
            adj_np = adj_sparse[sub_nodes][:, sub_nodes].toarray().astype(np.float32, copy=False)
            feat_ts = torch.from_numpy(feat_np).to(device)
            adj_ts = torch.from_numpy(adj_np).to(device)
            core_mask_ts = torch.from_numpy(core_mask).to(device=device)

            with torch.amp.autocast(device_type=device.type, enabled=(device.type == "cuda")):
                z_cell, z_niche = model.encode(feat_ts, adj_ts)
                if write_gene_outputs:
            ...
            z_cell_np[start:end] = z_cell[core_mask_ts].cpu().numpy()
            z_niche_np[start:end] = z_niche[core_mask_ts].cpu().numpy()
            ...
            if write_gene_outputs:
                ...
                x_input_np[start:end] = x_input_core.cpu().numpy()
                x_niche_np[start:end] = x_niche_core.cpu().numpy()
                if x_cell_np is not None:
                    x_cell_core = x_cell[core_mask_ts]
                    if selected_idx_t is not None:
                        x_cell_core = x_cell_core.index_select(1, selected_idx_t)
                    x_cell_np[start:end] = x_cell_core.cpu().numpy()
                else:
                    x_cell_core = None
            else:
                x_input_core = None
            ...
            del feat_np, adj_np, feat_ts, adj_ts, core_mask_ts, z_cell, z_niche, x_niche, x_cell
            del x_input_core, x_niche_core, x_cell_core
            if device.type == "cuda":
                torch.cuda.empty_cache()
            print(f"  progress: {end}/{n_cells} sub_nodes={sub_nodes.size}")
        ...
        del feat_sparse, adj_sparse, z_cell_np, z_niche_np, x_input_np, x_niche_np, x_cell_np, adata_out
        gc.collect()
