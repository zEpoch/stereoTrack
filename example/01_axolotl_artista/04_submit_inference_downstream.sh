...
RUNNER="${WORK_DIR}/example/01_axolotl_artista/04_run_inference_downstream.sh"
DEPENDENCY_JOB_ID="${1:-${DEPENDENCY_JOB_ID:-}}"
mkdir -p "${LOG_DIR}"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)

DSUB_ARGS=(
  -n "01_axolotl_down_infer"
  -A "root.project.P23Z10200N0876"
  -R "cpu=16;gpu=1;mem=160000"
  -T "172800"
  -N 1
  -oo "${LOG_DIR}/downstream_inference_${TIMESTAMP}.out"
  -eo "${LOG_DIR}/downstream_inference_${TIMESTAMP}.err"
)
...
echo "Downstream inference submitted. Logs: ${LOG_DIR}"
if [[ -n "${DEPENDENCY_JOB_ID}" ]]; then
