...
RUNNER="${WORK_DIR}/example/02_Chen_Cell_cortex_v2/03_run_inference_embeddings.sh"
DEPENDENCY_JOB_ID="${1:-${DEPENDENCY_JOB_ID:-}}"
mkdir -p "${LOG_DIR}"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)

DSUB_ARGS=(
  -n "02_macaque_cortex_v2_embed"
  -A "root.project.P23Z10200N0876"
  -R "cpu=16;gpu=1;mem=80000"
  -T "86400"
  -N 1
  -oo "${LOG_DIR}/embedding_${TIMESTAMP}.out"
  -eo "${LOG_DIR}/embedding_${TIMESTAMP}.err"
)
...
echo "Embedding inference submitted. Logs: ${LOG_DIR}"
if [[ -n "${DEPENDENCY_JOB_ID}" ]]; then
