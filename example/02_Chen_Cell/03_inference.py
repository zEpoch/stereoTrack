...
def preprocess(cfg, force=False):
    ...
    for i, batch in enumerate(batches):
        ...
        coords_full = temp.obsm[spatial_key]
        ...
        slice_info_list_full.append(slice_info_full)
        ...
        '''
        best_axis = np.argmax(axis_vars)
        axis_names = ['X', 'Y', 'Z']
        axis_name = axis_names[best_axis] if best_axis < len(axis_names) else f"Dim_{best_axis}"
        print(f"      沿最长轴 ({axis_name}) 顺序切分固定 Patch (size={patch_size}) ...", end=" ", flush=True)
        sorted_indices = np.argsort(coords[:, best_axis])
        print("done")
        n_patches_this = 0
        for start in range(0, n_cells, patch_size):
            end = min(start + patch_size, n_cells)
            cell_idx = sorted_indices[start:end]
            cell_idx = np.sort(cell_idx)
            if len(cell_idx) == 0:
                continue
        ...
        top_two_axes = np.argsort(axis_vars)[::-1][:2]
        axis_names = ['X', 'Y', 'Z']
        n_patches_this = 0
        for pass_idx, target_axis in enumerate(top_two_axes):
            axis_name = axis_names[target_axis] if target_axis < len(axis_names) else f"Dim_{target_axis}"
            print(f"      [第 {pass_idx+1}/2 次分块] 沿 ({axis_name}) 轴顺序切分固定 Patch (size={patch_size}) ...", end=" ", flush=True)
        ...
        temp.X = sp.csr_matrix(temp.shape, dtype=np.float16)
    ...
    meta = {
        ...
        "patches": slice_info_list,        # 给 train 读的固定物理小块
        "slice_info": slice_info_list_full # 给 inference 读的原生全图大小
    }
...
# python /home/share/huadjyin/home/zhoutao3/tracks/stereoTrack/example/02_Chen_Cell/03_inference.py --config /home/share/huadjyin/home/zhoutao3/tracks/stereoTrack/config/03_config_hanMouse3D_v6_3.yaml