...
MODEL_DIR="${WORK_DIR}/out/04_three_species_mae_v1_train_balanced_v1"
...
if [ ! -s "${CHECKPOINT_FILE}" ]; then
    ...
    echo "Submit inference after balanced training has finished."
    exit 1
fi
...
dsub \
    -n "04_three_species_infer_bal_last" \
...
echo "Balanced last.ckpt inference submitted; logs: ${LOG_DIR}"
