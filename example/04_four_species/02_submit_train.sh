#!/bin/bash
# ═══════════════════════════════════════════════════════════════
# dsub 提交脚本：04_four_species 训练任务
#
# 使用方式：
#   bash 02_submit_train.sh
#
# 默认会使用 config/04_config_four_species.yaml
# ═══════════════════════════════════════════════════════════════

WORK_DIR="/home/share/huadjyin/home/zhoutao3/tracks/stereoTrack"
LOG_DIR="${WORK_DIR}/out/04_four_species_mae_v1_train/job_logs"
CONFIG_FILE="${WORK_DIR}/config/04_config_four_species.yaml"
mkdir -p "${LOG_DIR}"

TIMESTAMP=$(date +%Y%m%d_%H%M%S)

dsub \
    -n "04_four_species_mae_v1" \
